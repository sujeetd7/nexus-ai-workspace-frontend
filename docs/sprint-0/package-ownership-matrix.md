...
| `shared-validation` | Shared validation contracts               | Zod schemas; Batch 1.3 `parsePublicClientConfig` for **plain objects**; returns `Result`/`AppError` from `shared-types` | API transport, UI, `import.meta.env`, `process.env`, platform APIs |
