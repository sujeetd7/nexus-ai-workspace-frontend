...
Sprint 3 Batch 3.2 — deterministic application bootstrap for Web and React Native.
...
6. renders providers only after critical initialization succeeds.
...
6. Resolve BootstrapOutcome
7. Render provider composition when ready
```
...
Transport clients remain available (web). No GraphQL React provider — deferred until an approved consumer requires one (ADR-0006).
